...
class FspvController extends Controller
{
    public function index()
    {
        abort_if(Gate::denies('form-spv.index'), Response::HTTP_FORBIDDEN, 'Forbidden');
        $userId = auth()->id();
        $bank = Bank::get();
        $norek = NoRek::all();
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('form-spv.index')
            ->with(
                'success',
    ...
    public function detail($id)
    {
        $show = Form::find($id);
        // dd($show);
        return view('pages.f-spv.detail', [
            'show'   => $show
        ]);
    }
