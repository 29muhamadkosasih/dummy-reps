...
class FmanController extends Controller
{
    public function index()
    {
        abort_if(Gate::denies('form-man.index'), Response::HTTP_FORBIDDEN, 'Forbidden');
        $userId = auth()->id();
        $bank = Bank::get();
        $norek = NoRek::all();
    ...
    public function detail($id)
    {
        $show = Form::find($id);
        // dd($show);
        return view('pages.f-man.detail', [
            'show'   => $show
        ]);
    }
